...
router.post('/:id/share', async (req: Request, res: Response) => {
  try {
    ...
    // Generate stable uuid_link if missing
...
async function uploadResume(userId: string, fileBase64: string, filename: string): Promise<string | null> {
  try {
    ...
    // Use Supabase Storage if configured (bucket: 'uploads')
...
router.post('/:id/apply', async (req: Request, res: Response) => {
  try {
    ...
    // Upload resume if provided (expect base64 string + filename in cover_note meta or name parsing)
    ...
    // Increment apply_clicks for the share if present
    try {
      if (share_uuid) {
    ...
    // Notify recruiter via email (simple)
    try {
