...
import sidebarAppsRouter from './src/routes/sidebarApps';
...
import widgetsRouter from './src/routes/widgets';
...
import hiringManagerInferRouter from './src/routes/hiringManagerInfer';
...
import igniteEventsRouter from './src/routes/ignite.events';
import igniteBackofficeRouter from './src/routes/ignite.backoffice';
...
app.use('/api/user', sidebarAppsRouter);
app.use('/api/ignite/backoffice', igniteBackofficeRouter);
app.use('/api/ignite/events', igniteEventsRouter);
...
app.use('/api/collaborators', requireAuthFlag, collaboratorsRouter);
  ...
  app.use('/api', hiringManagerInferRouter);
...
// Accept POST as well for older clients calling POST instead of PATCH
app.post('/api/sendgrid/update-sender', async (req, res) => {
  try {
    const { user_id, default_sender } = req.body as { user_id?: string; default_sender?: string };
    if (!user_id || !default_sender) {
      res.status(400).json({ error: 'Missing required fields' });
    ...
    console.error('Error updating SendGrid sender (POST):', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
// Start the server
