...
export default async function rexChat(req: Request, res: Response) {
  ...
  try {
    ...
    const tools: any = [
      ...
      { type:'function', function:{ name:'check_lead_overlap', description:'Check for email overlaps between sourcing campaigns. Shows which leads already exist in other active campaigns to prevent duplicate outreach.', parameters:{ type:'object', properties:{ userId:{type:'string'}, campaign_id:{type:'string',description:'Campaign to check leads from against other active campaigns'}, emails:{type:'array',items:{type:'string'},description:'Specific email addresses to check for overlap'} }, required:['userId'] } } },
      { type:'function', function:{ name:'create_ab_test', description:'Create an A/B test for a sequence step. Variant A = current content, Variant B = provided alternative.', parameters:{ type:'object', properties:{ userId:{type:'string'}, sequence_id:{type:'string'}, step_order:{type:'number'}, variant_b_subject:{type:'string'}, variant_b_body:{type:'string'}, primary_metric:{type:'string'}, min_sends:{type:'number'} }, required:['userId','sequence_id','step_order','variant_b_subject','variant_b_body'] } } },
      { type:'function', function:{ name:'get_ab_results', description:'Get A/B test results — per-variant open rates, reply rates, and winner recommendation.', parameters:{ type:'object', properties:{ userId:{type:'string'}, sequence_id:{type:'string'}, step_order:{type:'number'} }, required:['userId','sequence_id'] } } },
      { type:'function', function:{ name:'promote_ab_variant', description:'Manually promote an A/B test winner variant.', parameters:{ type:'object', properties:{ userId:{type:'string'}, test_id:{type:'string'}, variant_id:{type:'string'} }, required:['userId','test_id','variant_id'] } } },
      { type:'function', function:{ name:'get_pending_handoffs', description:'Get pending notification handoffs that need attention — unread replies, health alerts, milestones.', parameters:{ type:'object', properties:{ userId:{type:'string'}, type:{type:'string',description:'Optional filter: sourcing_reply, health_alert, milestone'} }, required:['userId'] } } },
      { type:'function', function:{ name:'resolve_handoff', description:'Take action on a pending notification handoff — draft a reply, book a meeting, disqualify a lead, pause a campaign.', parameters:{ type:'object', properties:{ userId:{type:'string'}, thread_key:{type:'string'}, action_id:{type:'string',description:'Action: reply_draft, book_meeting, disqualify, snooze, pause_campaign, resume_campaign, send_draft'}, data:{type:'object',description:'Optional data for the action'} }, required:['userId','thread_key','action_id'] } } }
    ];
