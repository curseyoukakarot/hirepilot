...
// Create a dedicated service-role client to avoid import path ambiguity between
// backend/lib/* and backend/src/lib/* in different deployment setups.
const SUPABASE_URL = process.env.SUPABASE_URL || '';
const SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || '';
if (!SUPABASE_URL || !SERVICE_KEY) {
  throw new Error('Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY');
}
const supabaseDb: SupabaseClient = createClient(SUPABASE_URL, SERVICE_KEY, {
