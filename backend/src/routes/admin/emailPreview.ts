...
router.get('/admin/email-preview/drip/:template', async (req: Request, res: Response) => {
  try {
    const template = String(req.params.template || '').trim();
    if (!template) {
      res.status(400).json({ error: 'template required' });
    ...
    const candidates = [
      path.resolve(process.cwd(), 'frontend', 'src', 'templates', 'emails', 'drips', `${template}.html`),
      path.resolve(__dirname, '../../../../frontend/src/templates/emails/drips', `${template}.html`)
    ];
    const filePath = candidates.find((p) => fs.existsSync(p));
    if (!filePath) {
      res.status(404).json({ error: 'template_not_found' });
