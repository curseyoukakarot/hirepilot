...
(async () => {
  ...
  try {
    ...
    console.log('Response preview:', JSON.stringify(json, null, 2).substring(0, 800));
    
    if (Array.isArray(json.elements)) {
      console.log('✅ SUCCESS: Elements found:', json.elements.length);
      console.log('First element sample:', JSON.stringify(json.elements[0], null, 2));
    } else if (json.data) {
      console.log('Response has data key - checking structure...');
      console.log('Data type:', typeof json.data);
      if (Array.isArray(json.data)) {
        console.log('Data is array with length:', json.data.length);
      }
    } else {
