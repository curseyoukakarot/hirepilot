...
export default function TaskDetailPanel({
  ...
  // ── Desktop: original side panel (flex child in row layout) ──

  if (isDesktop) {
    return (
      <div className="w-[400px] bg-white border-l border-gray-200 flex flex-col h-screen max-h-screen shadow-drawer z-30 sticky top-0 self-start overflow-hidden flex-shrink-0">
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 bg-white flex flex-col">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-gray-50 flex-shrink-0">
          ...
          <span className="text-sm font-semibold text-gray-800 truncate flex-1">{task.title}</span>
        ...
        {/* Code + date bar */}
        <div className="flex items-center gap-2 px-4 py-2 border-b border-gray-100 text-xs text-gray-500 flex-shrink-0">
          <span className="bg-gray-100 px-2 py-1 rounded-md border border-gray-200 font-mono text-gray-600">{task.code}</span>
          <span>•</span>
          <span>{task.createdAt}</span>
        </div>
