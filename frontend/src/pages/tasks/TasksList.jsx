...
export default function TasksList({ tasks, selectedTaskId, onSelectTask }) {
  return (
    <div className="bg-dark-300 shadow-card rounded-lg overflow-hidden flex-1 flex flex-col border border-gray-800 min-h-0">
      ...
      <div className="overflow-y-auto flex-1">
        {tasks.map((task) => (
          <TaskRow key={task.id} task={task} selected={selectedTaskId === task.id} onSelect={onSelectTask} />
        ))}
        {tasks.length === 0 && (
