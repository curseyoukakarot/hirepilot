...
import SalesAgentStrategyModal, { type Strategy } from '../../components/modals/SalesAgentStrategyModal';
...
export default function SalesAgentSettingsCard(){
  ...
  const [strategy, setStrategy] = useState<Strategy>({ tone:'professional', priority:'book', instructions:'' });
  ...
  useEffect(() => { (async()=>{
    try {
      ...
      try {
        const rs = (merged as any).response_strategy || (merged as any).reply_strategy || {};
        setStrategy({
          tone: merged.reply_style?.tone || (rs.tone || 'professional'),
          priority: rs.priority || 'book',
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          <button className="rounded bg-slate-800 hover:bg-slate-700 px-3 py-1.5 text-xs text-white" onClick={()=> setShowStrategyModal(true)} title="Open strategy modal to guide REX responses">Customize response strategy</button>
        </div>
      </div>
      ...
      <SalesAgentStrategyModal
        open={showStrategyModal}
        initial={strategy}
        onCancel={()=> setShowStrategyModal(false)}
        onSave={async (s: Strategy) => {
          if (!policy) return;
          // Persist via new endpoint
          try {
            await api('/api/sales-agent/strategy', { method: 'PATCH', body: JSON.stringify(s) });
            // Update local view-state and policy mirror
            setStrategy(s);
            const next = {
              ...policy,
              reply_style: { ...(policy.reply_style || {}), tone: s.tone },
              response_strategy: s,
              reply_strategy: s // keep legacy mirror for now
            };
            setPolicy(next);
            toast.success('Sales Agent Strategy saved.');
          } catch (e:any) {
