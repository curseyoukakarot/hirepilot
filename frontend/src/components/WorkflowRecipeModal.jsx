...
export default function WorkflowRecipeModal({
  ...
  const toolsGrid = useMemo(() => {
    ...
    return tools?.slice(0, 3).map((tool, idx) => {
      ...
      return (
        <div key={`${tool}-${idx}`} className="flex items-center space-x-3 p-4 bg-gray-50 dark:bg-slate-800 rounded-lg">
          {iconNode}
        <div>
          <div className="font-medium text-gray-900 dark:text-white">{tool}</div>
          <div className="text-sm text-gray-600 dark:text-slate-300">{subtitleFor(tool)}</div>
        </div>
        </div>
      );
    });
      ...
      {isOpen ? (
        ...
        >
          <motion.div
            role="dialog"
            aria-modal="true"
            className="bg-white dark:bg-slate-950 text-gray-900 dark:text-white rounded-2xl shadow-lg border border-gray-200 dark:border-slate-800 max-w-4xl mx-auto overflow-hidden w-full"
          ...
          >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 dark:text-slate-300">{summary}</p>
                  </div>
                </div>
                <div className="flex items-center space-x-3">
                  <div className="flex items-center space-x-2">
                    <span className="text-sm text-gray-600 dark:text-slate-300">Developer View</span>
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      />
                      <div className="w-11 h-6 bg-gray-200 dark:bg-slate-700 rounded-full peer peer-checked:bg-primary peer-focus:outline-none peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:h-5 after:w-5 after:rounded-full after:transition-all"></div>
                    </label>
                  </div>
                  <button ref={closeBtnRef} className="text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white" onClick={onClose} aria-label="Close">
            ...
            <div className="p-6">
              <div className="flex space-x-1 mb-6">
                {TABS.map((tab) => (
                  <button
                    ...
                    className={
                      tab === activeTab
                        ? 'px-4 py-2 bg-primary text-white rounded-lg font-medium shadow-sm'
                        : 'px-4 py-2 text-gray-500 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white rounded-lg transition-colors'
                    }
              ...
              {activeTab === 'Overview' && (
                <div id="modal-content" className="space-y-6">
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      <h4 className="font-semibold text-gray-900 dark:text-white mb-2">What This Does</h4>
                      <p className="text-gray-700 dark:text-slate-300">{summary}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-4 bg-gradient-to-r from-primary/10 to-secondary/10 rounded-lg border border-primary/20">
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white">Setup Time: {setupTime}</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <i className="fa-solid fa-star text-warning"></i>
                      <span className="font-medium text-gray-900 dark:text-white">Difficulty: {difficulty}</span>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4">
                  {setupSteps?.length ? (
                    setupSteps.map((step, idx) => (
                      <div key={idx} className="flex items-start space-x-3 p-4 bg-gray-50 dark:bg-slate-800 rounded-lg">
                        <div className="w-8 h-8 bg-gradient-to-br from-primary to-secondary rounded-md text-gray-900 flex items-center justify-center font-semibold">
                          {idx + 1}
                        </div>
                        <div className="text-gray-900 dark:text-white">{step}</div>
                      </div>
                    ))
                  ) : (
                    <div className="text-gray-500 dark:text-slate-400">No setup steps provided.</div>
                  )}
                </div>
              )}
              ...
              {activeTab === 'Formula' && (
                <div className="space-y-4">
                  {devMode ? (
                    <div className="bg-gray-100 dark:bg-slate-900 rounded-xl p-4 text-gray-900 dark:text-slate-100">
                      <pre className="code-block"><code className="language-yaml">{formula}</code></pre>
                    </div>
                  ) : (
                    <div className="p-4 bg-gray-100 dark:bg-slate-900 rounded-lg text-gray-700 dark:text-slate-200">
            ...
            {/* Modal Footer */}
            <div className="bg-gray-50 dark:bg-slate-900 border-t border-gray-200 dark:border-slate-800 p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className="w-2 h-2 bg-success rounded-full"></div>
                  <span className="text-sm text-gray-600 dark:text-slate-300">Ready to deploy</span>
                </div>
                <div className="flex space-x-3">
                  <button className="border border-gray-300 dark:border-slate-700 text-gray-700 dark:text-slate-200 hover:bg-gray-100 dark:hover:bg-slate-800 px-6 py-2 rounded-lg font-medium transition-all">
                    Preview
                  </button>
