...
export default function ConnectLinkedInModal({ open, onClose, userId }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    setStatus('starting');
    fetch(`${API_BASE}/linkedin/session/start`, {
      method: 'POST',
  ...
  async function complete() {
    if (!sessionId) return;
    const r = await fetch(`${API_BASE}/linkedin/session/complete`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-user-id': userId },
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-5xl shadow-xl overflow-hidden">
        ...
        <div className="p-0">
          {streamUrl ? (
            (() => {
              const origin = (() => { try { return new URL(streamUrl).origin; } catch { return 'https://stream.thehirepilot.com'; } })();
              const noVncUrl = `${origin}/vnc_lite.html?path=websockify&encrypt=1&autoconnect=1&resize=scale`;
              return (
                <iframe
                  src={noVncUrl}
