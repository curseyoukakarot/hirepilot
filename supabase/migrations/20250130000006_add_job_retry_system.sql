-- Job Retry System with Exponential Backoff and Failed Job Sweeper
-- Handles job failures with intelligent retry logic and permanent failure management
...
SELECT 
  j.*,
  jrh.last_attempt_at,
  jrh.last_failure_reason,
  jrh.total_retry_attempts,
...
LEFT JOIN LATERAL (
  SELECT 
    ...
    FIRST_VALUE(failure_reason) OVER (ORDER BY started_at DESC) as last_failure_reason
